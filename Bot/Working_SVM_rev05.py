...
def creating_binary_labels(close_list, open_price_list):
    ...
    if label_pos == 'True':
        label_list = close_list - [x*1.0018264 for x in open_price_list]        #0018264
    else:
        label_list = open_price_list - [x*1.0018264 for x in close_list]
...
if __name__ == '__main__':
    ...
    fp2= open("dataset/HistoryData.csv",'r+', encoding = 'utf-16')
    choice=2               #int(input("choose URL(1) or file(2) :"))
    if choice==1:
...
#        gamma_val = 0.028
    ...
    feature_next = fearure_next(timestamp_list, close_list, high_list, low_list, 
                                                    open_price_list, volume_list, x )
    
    predicted_next = clf.predict(feature_next[-10:])
    print("New: ",predicted_next)
